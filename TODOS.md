...
- [ ] **캘린더 자동 추가** — 상세 화면 "캘린더" 버튼 탭 → 권한 요청 → 캘린더 앱에 이벤트 등록 확인
- [ ] **OCR 스캔** — "카메라 촬영" / "갤러리 선택" → 청첩장 이미지 인식 → 신랑/신부/날짜/장소 자동 입력 확인
- [ ] **모바일 청첩장 파싱** — 청첩장 URL 입력 → "불러오기" → parse-invitation Edge Function 호출 → 폼 자동 완성 확인
